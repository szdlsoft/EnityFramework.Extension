...
namespace EntityFramework.BulkInsert.Helpers
{
    public class MappedDataReader<T> : IDataReader
    {
            ...
            foreach (var kvp in tableMappings)
            {
                ...
                foreach (var col in propertyMaps)
                {
                    ...
                    if (col.IsNavigationProperty)
                    {
                        ...
                        Expression propertyReferExpression = Expression.PropertyOrField(propertyExpression, prop.Name);
                        // 目前支持的Id 类型为int32 和int64
                        Expression zeroId = Expression.Constant( prop.PropertyType == typeof(int) ?  0 : 0L,  prop.PropertyType); //默认id = 0

                        Expression nullSafeExpression = Expression.IfThenElse( Expression.Equal(propertyExpression, Expression.Constant(null))
                                                                                , zeroId
                                                                                , propertyReferExpression);


                        var expression = Expression.Lambda<Func<T, object>>(Expression.Convert(propertyReferExpression, typeof(object)), x);
        ...
        private static Dictionary<Type, PropertyInfo> ReferenceEntityIdProps = new Dictionary<Type, PropertyInfo>();
